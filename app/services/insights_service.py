...
class InsightsService:
    ...
    @staticmethod
    async def create_insight_light(
        url: str,
        ...
        try:
            ...
            final_title = title or f"来自 {domain}" if domain else "新见解"
            
            # 准备轻量级insight数据
            insight_insert_data = {
                'title': final_title,
                'description': '',  # 空描述，等待异步填充
                'url': url,
                'image_url': None,  # 空图片，等待异步填充
                'user_id': str(user_id),
                'tags': [],  # 新的JSONB字段，初始为空数组
                'stack_id': stack_id,
                'status': 'queued',  # 新增状态字段
                'meta': None  # 空metadata，等待异步填充
            }
            
            logger.info(f"🚀 轻写入模式：创建insight - url={url}, title={final_title}")
            
            # 创建insight记录
            response = supabase_service.table('insights').insert(insight_insert_data).execute()
            
            if hasattr(response, 'error') and response.error:
                logger.error(f"轻写入创建insight失败: {response.error}")
            ...
            if tag_ids:
                tags_result = await InsightTagService.update_insight_tags_by_ids(
                    insight_id, tag_ids, user_id
                )
                if not tags_result.get('success'):
                    logger.warning(f"轻写入创建insight成功，但标签处理失败: {tags_result.get('message')}")
            
            # 处理thought字段（保存到insight_contents表）
            if thought:
                try:
                    content_payload = {
                    ...
                    if hasattr(content_res, 'error') and content_res.error:
                        logger.warning(f"保存thought失败: {content_res.error}")
                except Exception as thought_err:
                    logger.warning(f"处理thought字段失败: {thought_err}")
            
            # 返回轻量级响应
            return {
                "success": True,
                "message": "Insight创建成功（metadata处理中）",
                "data": {
                    "id": insight['id'],
                    ...
                    "url": insight['url'],
                    "image_url": insight.get('image_url'),
                    "stack_id": insight.get('stack_id'),
                    "status": insight.get('status', 'queued'),
                    "created_at": insight['created_at'],
                    "updated_at": insight['updated_at'],
                    "tags": []
                }
            ...
            logger.error(f"轻写入创建insight失败: {str(e)}")
            return {"success": False, "message": f"创建insight失败: {str(e)}"}

    @staticmethod
    async def _fetch_metadata_async(insight_id: UUID, url: str, user_id: UUID) -> None:
        """异步抓取metadata并更新insight记录"""
        try:
            logger.info(f"[异步任务] 开始抓取metadata: insight_id={insight_id}, url={url}")
            
            # 更新状态为处理中
            supabase_service = get_supabase_service()
            await supabase_service.table('insights').update({
                'status': 'processing'
            }).eq('id', str(insight_id)).execute()
            
            # 抓取metadata
            try:
                from app.utils.metadata import extract_metadata_from_url
                metadata = await extract_metadata_from_url(url)
                logger.info(f"[异步任务] metadata抓取成功: {url}")
            except Exception as fetch_err:
                logger.warning(f"[异步任务] metadata抓取失败: {fetch_err}")
                metadata = {
                    "title": "无标题",
                    "description": "",
                    "image_url": None
            ...
            response = await supabase_service.table('insights').update(update_data).eq('id', str(insight_id)).execute()
            
            if hasattr(response, 'error') and response.error:
                logger.error(f"[异步任务] 更新insight失败: {response.error}")
                # 更新状态为失败
                await supabase_service.table('insights').update({
                    'status': 'failed'
                }).eq('id', str(insight_id)).execute()
            else:
                logger.info(f"[异步任务] metadata更新成功: insight_id={insight_id}")
                ...
                if os.getenv('FETCH_PAGE_CONTENT_ENABLED', '').lower() in ('1', 'true', 'yes'):
                    try:
                        ...
                        asyncio.create_task(InsightsService._fetch_and_save_content(
                            ...
                            user_id=user_id,
                            url=url,
                            thought=None  # thought已经在轻写入时保存了
                        ))
                        logger.info(f"[异步任务] 已启动内容抓取任务: insight_id={insight_id}")
                    except Exception as content_err:
                        logger.warning(f"[异步任务] 启动内容抓取任务失败: {content_err}")
                
        except Exception as e:
            logger.error(f"[异步任务] metadata抓取异常: {e}")
            # 更新状态为失败
            try:
                supabase_service = get_supabase_service()
                await supabase_service.table('insights').update({
                ...
                logger.error(f"[异步任务] 更新失败状态也失败: {update_err}")

    @staticmethod
    async def get_insight_status(insight_id: UUID, user_id: UUID) -> Dict[str, Any]:
        """获取insight的处理状态和最新数据"""
        try:
            supabase_service = get_supabase_service()
            ...
            if hasattr(response, 'error') and response.error:
                logger.error(f"获取insight状态失败: {response.error}")
                return {"success": False, "message": "获取insight状态失败"}
            ...
            logger.error(f"获取insight状态失败: {str(e)}")
            return {"success": False, "message": f"获取insight状态失败: {str(e)}"}
