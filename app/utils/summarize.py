...
async def generate_summary(text: str) -> Optional[str]:
    ...
    try:
        ...
        prompt_system = (
            ...
            "Always return a usable summary — never output an empty string.\n"
            "If parts of the content are sensitive or disallowed, replace only that portion with a bracketed placeholder "
            "(e.g., \"[redacted PII]\", \"[political content omitted]\") but continue summarizing the rest.\n\n"
            "Rules:\n"
            "1) Target length: aim for 2–4 sentences. Expand slightly (up to ~10) if needed to preserve important facts.\n"
            "2) Keep the original language of the input (do not translate).\n"
            "3) Extract key facts, entities (names, organizations, products), dates, and numbers. Preserve exact values when present.\n"
            "4) Focus on what happened / who is involved / when / where / why / how. Conciseness is preferred, but accuracy and completeness of critical facts take priority.\n"
            "5) Exclude speculation, opinions, or new claims not in the source.\n"
            "6) Omit navigation, boilerplate, ads, cookie banners, and comments.\n"
            "7) For technical/academic text, emphasize purpose, methods, and results. For code/logs/configs, summarize purpose, main components, and notable issues.\n"
            "8) Redact PII or sensitive details with placeholders, but never drop the entire summary.\n"
            "9) If the text is mostly noise or unreadable, output one sentence noting this plus any clearly identifiable topic (do not invent one).\n"
            "10) Return plain text only — no bullets, no markdown, no headings, no formatting."
        )
